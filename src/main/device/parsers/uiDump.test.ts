...
describe('parseUiDump', () => {
  ...
  it('filters by query across text, contentDesc and resourceId, case-insensitively', () => {
    const byText = parseUiDump(sample, { ...screen, query: '로그인' })
    expect(byText.map((node) => node.resourceId)).toEqual(['login'])

    const byId = parseUiDump(sample, { ...screen, query: 'EMAIL' })
  ...
  it('parses the recorded real-device fixture without throwing and returns tappable nodes', () => {
    const nodes = parseUiDump(realFixture, realScreen)

    expect(nodes.length).toBeGreaterThan(0)
    for (const node of nodes) {
      expect(node.x).toBeGreaterThanOrEqual(0)
      expect(node.x).toBeLessThanOrEqual(realScreen.screenWidth)
      expect(node.y).toBeGreaterThanOrEqual(0)
      expect(node.y).toBeLessThanOrEqual(realScreen.screenHeight)
    }
  })
})
